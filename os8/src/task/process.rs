...
use super::{add_task, pid_alloc, PidHandle, TaskControlBlock,TaskStatus};
...
pub struct ProcessControlBlockInner {
    ...
    pub sem_alloc: Vec<Vec<usize>>,     //(tid,sem) -> alloc
    pub sem_avil: Vec<usize>,           //sem -> count
    pub sem_request: Vec<Option<usize>>, //tid -> sem
}
...
impl ProcessControlBlockInner {
    ...
    pub fn get_current_thread(&self)->usize{
        self.tasks.iter().enumerate()
        .find(|(_,item)| item.is_some()&&item.as_ref().unwrap().inner_exclusive_access().get_status()==TaskStatus::Running)
        .map(|(id,_)|id).unwrap()
